...
- **Z-order** (bottom→top): basemap fills < incident-map overlay < spread forecast < weather rasters < traffic < basemap labels < IR heat < perimeter fill/line < hotspots < pins. Enforced by `zOrder.ensureOrder()`. Rasters stack live-over-static: the incident-map sheet is an opaque scan and floors the group, and weather paints over the spread forecast, so neither a briefing PDF nor a forecast at full opacity can hide the current conditions read on top of it.
