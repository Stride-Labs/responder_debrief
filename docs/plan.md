...
- **Z-order** (bottom→top): basemap fills < incident-map overlay < spread forecast < weather rasters < basemap labels < IR heat < perimeter fill/line < hotspots < pins. Enforced by `zOrder.ensureOrder()`.
