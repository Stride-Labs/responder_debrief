...
- **Z-order** (bottom→top): basemap fills < incident-map overlay < spread forecast < weather rasters < traffic < basemap labels < IR heat < perimeter fill/line < hotspots < pins. Enforced by `zOrder.ensureOrder()`. Weather sits above the incident-map sheet and the forecast so a full-opacity map overlay can't erase the wash underneath it; `zOrder.ts` keeps the below-label rasters and above-label vectors in separate arrays so nothing drifts across the label seam.
